...
public:
	...
	friend bool QMdiArea_virtualbase_nativeEvent(VirtualQMdiArea* self, struct seaqt_string eventType, void* message, intptr_t* result);
	...
	friend double QMdiArea_protectedbase_getDecodedMetricF(const VirtualQMdiArea* self, int metricA, int metricB);
};
...
double QMdiArea_protectedbase_getDecodedMetricF(const VirtualQMdiArea* self, int metricA, int metricB) {
	return self->QMdiArea::getDecodedMetricF(static_cast<VirtualQMdiArea::PaintDeviceMetric>(metricA), static_cast<VirtualQMdiArea::PaintDeviceMetric>(metricB));
}
