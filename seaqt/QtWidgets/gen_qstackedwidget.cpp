...
public:
	...
	friend double QStackedWidget_protectedbase_getDecodedMetricF(const VirtualQStackedWidget* self, int metricA, int metricB);
};
...
void QStackedWidget_connect_widgetAdded(QStackedWidget* self, intptr_t slot, void (*callback)(intptr_t, int), void (*release)(intptr_t)) {
	QStackedWidget::connect(self, static_cast<void (QStackedWidget::*)(int)>(&QStackedWidget::widgetAdded), self, [callback, release = seaqt::release_callback{slot,release}](int index) {
			int sigval1 = index;
			callback(release.slot, sigval1);
	});
}
...
double QStackedWidget_protectedbase_getDecodedMetricF(const VirtualQStackedWidget* self, int metricA, int metricB) {
	return self->QStackedWidget::getDecodedMetricF(static_cast<VirtualQStackedWidget::PaintDeviceMetric>(metricA), static_cast<VirtualQStackedWidget::PaintDeviceMetric>(metricB));
}
