...
public:
	...
	friend struct seaqt_string QFileIconProvider_virtualbase_type(const void* self, QFileInfo* info);
...
struct seaqt_string QFileIconProvider_virtualbase_type(const void* self, QFileInfo* info) {
	QString _ret = static_cast<const VirtualQFileIconProvider*>(self)->QFileIconProvider::type(*info);
	// Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
	QByteArray _b = _ret.toUtf8();
	struct seaqt_string _ms;
	_ms.len = _b.length();
