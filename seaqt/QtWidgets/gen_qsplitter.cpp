...
VirtualQSplitter* QSplitter_new2(const QSplitter_VTable* vtbl, size_t vdata, int param1) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSplitter>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSplitter(vtbl, static_cast<Qt::Orientation>(param1)) : nullptr;
}

VirtualQSplitter* QSplitter_new3(const QSplitter_VTable* vtbl, size_t vdata, QWidget* parent) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSplitter>() + vdata, std::nothrow);
