...
struct seaqt_array /* of int */  QPrinterInfo_supportedPaperSizes(const QPrinterInfo* self) {
...
struct seaqt_array /* of struct seaqt_map  tuple of struct seaqt_string and QSizeF*   */  QPrinterInfo_supportedSizesWithNames(const QPrinterInfo* self) {
	QList<QPair<QString, QSizeF>> _ret = self->supportedSizesWithNames();
	// Convert QList<> from C++ memory to manually-managed C memory
	struct seaqt_map /* tuple of struct seaqt_string and QSizeF* */ * _arr = static_cast<struct seaqt_map /* tuple of struct seaqt_string and QSizeF* */ *>(malloc(sizeof(struct seaqt_map /* tuple of struct seaqt_string and QSizeF* */ ) * _ret.length()));
	for (size_t i = 0, e = _ret.length(); i < e; ++i) {
