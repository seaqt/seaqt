...
struct seaqt_array /* of QSize* */  miqt_exec_callback_QIconEngine_availableSizes(const QIconEngine*, intptr_t, int, int);
struct seaqt_string miqt_exec_callback_QIconEngine_iconName(const QIconEngine*, intptr_t);
...
public:
	...
	friend struct seaqt_array /* of QSize* */  QIconEngine_virtualbase_availableSizes(const void* self, int mode, int state);
	...
	friend struct seaqt_string QIconEngine_virtualbase_iconName(const void* self);
...
QIconEngine* QIconEngine_new2(QIconEngine* other) {
	return new (std::nothrow) VirtualQIconEngine(*other);
}
...
struct seaqt_array /* of QSize* */  QIconEngine_virtualbase_availableSizes(const void* self, int mode, int state) {
	QList<QSize> _ret = static_cast<const VirtualQIconEngine*>(self)->QIconEngine::availableSizes(static_cast<QIcon::Mode>(mode), static_cast<QIcon::State>(state));
	// Convert QList<> from C++ memory to manually-managed C memory
	QSize** _arr = static_cast<QSize**>(malloc(sizeof(QSize*) * _ret.length()));
	for (size_t i = 0, e = _ret.length(); i < e; ++i) {
		_arr[i] = new QSize(_ret[i]);
...
struct seaqt_string QIconEngine_virtualbase_iconName(const void* self) {
	QString _ret = static_cast<const VirtualQIconEngine*>(self)->QIconEngine::iconName();
	// Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
	QByteArray _b = _ret.toUtf8();
	struct seaqt_string _ms;
	_ms.len = _b.length();
