...
struct seaqt_array /* of QFont__Tag* */  QFont_featureTags(const QFont* self) {
	QList<QFont::Tag> _ret = self->featureTags();
	// Convert QList<> from C++ memory to manually-managed C memory
	QFont__Tag** _arr = static_cast<QFont__Tag**>(malloc(sizeof(QFont__Tag*) * _ret.length()));
	for (size_t i = 0, e = _ret.length(); i < e; ++i) {
...
struct seaqt_array /* of QFont__Tag* */  QFont_variableAxisTags(const QFont* self) {
...
QFont__Tag* QFont__Tag_new() {
	return new (std::nothrow) QFont__Tag();
}

QFont__Tag* QFont__Tag_new_from(QFont__Tag* from) {
	return new (std::nothrow) QFont__Tag(*from);
}
...
struct seaqt_string QFont__Tag_toString(const QFont__Tag* self) {
...
void QFont__Tag_operatorAssign(QFont__Tag* self, QFont__Tag* from) {
	self->operator=(*from);
}
