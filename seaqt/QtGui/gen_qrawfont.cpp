...
struct seaqt_string QRawFont_glyphName(const QRawFont* self, unsigned int glyphIndex) {
	QString _ret = self->glyphName(static_cast<quint32>(glyphIndex));
	// Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
	QByteArray _b = _ret.toUtf8();
	struct seaqt_string _ms;
	_ms.len = _b.length();
...
struct seaqt_string QRawFont_fontTable_tagName(const QRawFont* self, const char* tagName) {
...
struct seaqt_string QRawFont_fontTable_tag(const QRawFont* self, QFont__Tag* tag) {
