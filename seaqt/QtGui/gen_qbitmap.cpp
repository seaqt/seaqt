...
public:
	...
	VirtualQBitmap(const QBitmap_VTable* vtbl, const QBitmap& other): QBitmap(other), vtbl(vtbl) {}
...
VirtualQBitmap* QBitmap_new6(const QBitmap_VTable* vtbl, size_t vdata, QBitmap* other) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQBitmap>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQBitmap(vtbl, *other) : nullptr;
}
