...
struct seaqt_array /* of QGlyphRun* */  QTextLayout_glyphRuns_from_length_flags(const QTextLayout* self, int from, int length, uint16_t flags) {
...
struct seaqt_array /* of QGlyphRun* */  QTextLine_glyphRuns_from_length_flags(const QTextLine* self, int from, int length, uint16_t flags) {
	QList<QGlyphRun> _ret = self->glyphRuns(static_cast<int>(from), static_cast<int>(length), static_cast<QTextLayout::GlyphRunRetrievalFlags>(flags));
	// Convert QList<> from C++ memory to manually-managed C memory
	QGlyphRun** _arr = static_cast<QGlyphRun**>(malloc(sizeof(QGlyphRun*) * _ret.length()));
	for (size_t i = 0, e = _ret.length(); i < e; ++i) {
...
QTextLayout__FormatRange* QTextLayout__FormatRange_new() {
	return new (std::nothrow) QTextLayout__FormatRange();
}

QTextLayout__FormatRange* QTextLayout__FormatRange_new_from(QTextLayout__FormatRange* from) {
	return new (std::nothrow) QTextLayout__FormatRange(*from);
}
