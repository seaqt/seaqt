...
void QInputDevice_capabilitiesChanged(QInputDevice* self, int capabilities) {
	self->capabilitiesChanged(static_cast<QInputDevice::Capabilities>(capabilities));
}

void QInputDevice_connect_capabilitiesChanged(QInputDevice* self, intptr_t slot, void (*callback)(intptr_t, int), void (*release)(intptr_t)) {
	QInputDevice::connect(self, static_cast<void (QInputDevice::*)(QInputDevice::Capabilities)>(&QInputDevice::capabilitiesChanged), self, [callback, release = seaqt::release_callback{slot,release}](QInputDevice::Capabilities capabilities) {
			QInputDevice::Capabilities capabilities_ret = capabilities;
			int sigval1 = static_cast<int>(capabilities_ret);
			callback(release.slot, sigval1);
	});
}
