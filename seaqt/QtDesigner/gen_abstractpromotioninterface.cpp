...
struct seaqt_array /* set of struct seaqt_string */  QDesignerPromotionInterface_referencedPromotedClassNames(const QDesignerPromotionInterface* self) {
	QSet<QString> _ret = self->referencedPromotedClassNames();
	// Convert QSet<> from C++ memory to manually-managed C memory
	struct seaqt_string* _arr = static_cast<struct seaqt_string*>(malloc(sizeof(struct seaqt_string) * _ret.size()));
	int _ctr = 0;
...
struct seaqt_array /* of QDesignerWidgetDataBaseItemInterface* */  QDesignerPromotionInterface_promotionBaseClasses(const QDesignerPromotionInterface* self) {
