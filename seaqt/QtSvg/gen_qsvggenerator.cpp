...
public:
	...
	VirtualQSvgGenerator(const QSvgGenerator_VTable* vtbl, QSvgGenerator::SvgVersion version): QSvgGenerator(version), vtbl(vtbl) {}
	...
	virtual void initPainter(QPainter* param1) const override {
		if (vtbl->initPainter == 0) {
			QSvgGenerator::initPainter(param1);
			return;
		}
	...
	friend void QSvgGenerator_virtualbase_initPainter(const VirtualQSvgGenerator* self, QPainter* param1);
	...
	friend double QSvgGenerator_protectedbase_getDecodedMetricF(const VirtualQSvgGenerator* self, int metricA, int metricB);
};
...
VirtualQSvgGenerator* QSvgGenerator_new_version(const QSvgGenerator_VTable* vtbl, size_t vdata, int version) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSvgGenerator>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSvgGenerator(vtbl, static_cast<QSvgGenerator::SvgVersion>(version)) : nullptr;
}
...
int QSvgGenerator_svgVersion(const QSvgGenerator* self) {
	QSvgGenerator::SvgVersion _ret = self->svgVersion();
...
void QSvgGenerator_virtualbase_initPainter(const VirtualQSvgGenerator* self, QPainter* param1) {

	self->QSvgGenerator::initPainter(param1);
}
...
double QSvgGenerator_protectedbase_getDecodedMetricF(const VirtualQSvgGenerator* self, int metricA, int metricB) {
	return self->QSvgGenerator::getDecodedMetricF(static_cast<VirtualQSvgGenerator::PaintDeviceMetric>(metricA), static_cast<VirtualQSvgGenerator::PaintDeviceMetric>(metricB));
}
