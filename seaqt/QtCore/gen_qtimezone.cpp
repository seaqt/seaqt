...
QTimeZone* QTimeZone_new_spec(int spec) {
	return new (std::nothrow) QTimeZone(static_cast<QTimeZone::Initialization>(spec));
}
...
bool QTimeZone_hasAlternativeName(const QTimeZone* self, struct seaqt_string alias) {
	QByteArrayView alias_QByteArray(alias.data, alias.len);
	return self->hasAlternativeName(alias_QByteArray);
}
...
QTimeZone__OffsetData* QTimeZone__OffsetData_new_from(QTimeZone__OffsetData* from) {
	return new (std::nothrow) QTimeZone__OffsetData(*from);
}

QTimeZone__OffsetData* QTimeZone__OffsetData_new() {
	return new (std::nothrow) QTimeZone__OffsetData();
}
