#pragma once
#ifndef SEAQT_QTCORE_GEN_QFILEINFO_H
#define SEAQT_QTCORE_GEN_QFILEINFO_H

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#pragma GCC diagnostic ignored "-Wdeprecated-declarations"

#include "../libseaqt-runtime.h"

#ifdef __cplusplus
extern "C" {
#endif

#ifdef __cplusplus
class QDateTime;
class QDir;
class QFileDevice;
class QFileInfo;
class QTimeZone;
#else
typedef struct QDateTime QDateTime;
typedef struct QDir QDir;
typedef struct QFileDevice QFileDevice;
typedef struct QFileInfo QFileInfo;
typedef struct QTimeZone QTimeZone;
#endif

QFileInfo* QFileInfo_new();
QFileInfo* QFileInfo_new_QString(struct seaqt_string file);
QFileInfo* QFileInfo_new_QFileDevice(QFileDevice* file);
QFileInfo* QFileInfo_new_QDir_QString(QDir* dir, struct seaqt_string file);
QFileInfo* QFileInfo_new_QFileInfo(QFileInfo* from);

void QFileInfo_operatorAssign(QFileInfo* self, QFileInfo* from);
void QFileInfo_swap(QFileInfo* self, QFileInfo* other);
void QFileInfo_setFile_QString(QFileInfo* self, struct seaqt_string file);
void QFileInfo_setFile_QFileDevice(QFileInfo* self, QFileDevice* file);
void QFileInfo_setFile_QDir_QString(QFileInfo* self, QDir* dir, struct seaqt_string file);
bool QFileInfo_exists(const QFileInfo* self);
bool QFileInfo_exists_file(struct seaqt_string file);
void QFileInfo_refresh(QFileInfo* self);
struct seaqt_string QFileInfo_filePath(const QFileInfo* self);
struct seaqt_string QFileInfo_absoluteFilePath(const QFileInfo* self);
struct seaqt_string QFileInfo_canonicalFilePath(const QFileInfo* self);
struct seaqt_string QFileInfo_fileName(const QFileInfo* self);
struct seaqt_string QFileInfo_baseName(const QFileInfo* self);
struct seaqt_string QFileInfo_completeBaseName(const QFileInfo* self);
struct seaqt_string QFileInfo_suffix(const QFileInfo* self);
struct seaqt_string QFileInfo_bundleName(const QFileInfo* self);
struct seaqt_string QFileInfo_completeSuffix(const QFileInfo* self);
struct seaqt_string QFileInfo_path(const QFileInfo* self);
struct seaqt_string QFileInfo_absolutePath(const QFileInfo* self);
struct seaqt_string QFileInfo_canonicalPath(const QFileInfo* self);
QDir* QFileInfo_dir(const QFileInfo* self);
QDir* QFileInfo_absoluteDir(const QFileInfo* self);
bool QFileInfo_isReadable(const QFileInfo* self);
bool QFileInfo_isWritable(const QFileInfo* self);
bool QFileInfo_isExecutable(const QFileInfo* self);
bool QFileInfo_isHidden(const QFileInfo* self);
bool QFileInfo_isNativePath(const QFileInfo* self);
bool QFileInfo_isRelative(const QFileInfo* self);
bool QFileInfo_isAbsolute(const QFileInfo* self);
bool QFileInfo_makeAbsolute(QFileInfo* self);
bool QFileInfo_isFile(const QFileInfo* self);
bool QFileInfo_isDir(const QFileInfo* self);
bool QFileInfo_isSymLink(const QFileInfo* self);
bool QFileInfo_isSymbolicLink(const QFileInfo* self);
bool QFileInfo_isOther(const QFileInfo* self);
bool QFileInfo_isShortcut(const QFileInfo* self);
bool QFileInfo_isAlias(const QFileInfo* self);
bool QFileInfo_isJunction(const QFileInfo* self);
bool QFileInfo_isRoot(const QFileInfo* self);
bool QFileInfo_isBundle(const QFileInfo* self);
struct seaqt_string QFileInfo_symLinkTarget(const QFileInfo* self);
struct seaqt_string QFileInfo_readSymLink(const QFileInfo* self);
struct seaqt_string QFileInfo_junctionTarget(const QFileInfo* self);
struct seaqt_string QFileInfo_owner(const QFileInfo* self);
unsigned int QFileInfo_ownerId(const QFileInfo* self);
struct seaqt_string QFileInfo_group(const QFileInfo* self);
unsigned int QFileInfo_groupId(const QFileInfo* self);
bool QFileInfo_permission(const QFileInfo* self, int permissions);
int QFileInfo_permissions(const QFileInfo* self);
long long QFileInfo_size(const QFileInfo* self);
QDateTime* QFileInfo_birthTime(const QFileInfo* self);
QDateTime* QFileInfo_metadataChangeTime(const QFileInfo* self);
QDateTime* QFileInfo_lastModified(const QFileInfo* self);
QDateTime* QFileInfo_lastRead(const QFileInfo* self);
QDateTime* QFileInfo_fileTime_time(const QFileInfo* self, int time);
QDateTime* QFileInfo_birthTime_tz(const QFileInfo* self, QTimeZone* tz);
QDateTime* QFileInfo_metadataChangeTime_tz(const QFileInfo* self, QTimeZone* tz);
QDateTime* QFileInfo_lastModified_tz(const QFileInfo* self, QTimeZone* tz);
QDateTime* QFileInfo_lastRead_tz(const QFileInfo* self, QTimeZone* tz);
QDateTime* QFileInfo_fileTime_time_tz(const QFileInfo* self, int time, QTimeZone* tz);
bool QFileInfo_caching(const QFileInfo* self);
void QFileInfo_setCaching(QFileInfo* self, bool on);
void QFileInfo_stat(QFileInfo* self);

void QFileInfo_delete(QFileInfo* self);

#ifdef __cplusplus
} /* extern C */
#endif

#endif
