...
struct seaqt_string QMetaMethod_nameView(const QMetaMethod* self) {
	QByteArrayView _qb = self->nameView();
	struct seaqt_string _ms;
	_ms.len = _qb.length();
	_ms.data = static_cast<char*>(malloc(_ms.len));
	memcpy(_ms.data, _qb.data(), _ms.len);
...
struct seaqt_string QMetaEnum_valueToKeys(const QMetaEnum* self, unsigned long long value) {
...
QMetaClassInfo* QMetaClassInfo_new_from(QMetaClassInfo* from) {
	return new (std::nothrow) QMetaClassInfo(*from);
}
