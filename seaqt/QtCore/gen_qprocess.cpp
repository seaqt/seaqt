...
public:
	...
	friend long long QProcess_virtualbase_skipData(VirtualQProcess* self, long long maxSize);
...
void QProcess_failChildProcessModifier_description(QProcess* self, const char* description) {
	self->failChildProcessModifier(description);
}

void QProcess_setUnixProcessParameters_QProcess_UnixProcessFlags(QProcess* self, uint32_t flagsOnly) {
...
void QProcess_failChildProcessModifier_description_error(QProcess* self, const char* description, int error) {
...
long long QProcess_virtualbase_skipData(VirtualQProcess* self, long long maxSize) {
	...
	qint64 _ret = self->QProcess::skipData(static_cast<qint64>(maxSize));
	return static_cast<long long>(_ret);
}
