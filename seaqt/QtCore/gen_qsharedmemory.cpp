...
public:
	...
	VirtualQSharedMemory(const QSharedMemory_VTable* vtbl, const QNativeIpcKey& key): QSharedMemory(key), vtbl(vtbl) {}
	...
	VirtualQSharedMemory(const QSharedMemory_VTable* vtbl, const QNativeIpcKey& key, QObject* parent): QSharedMemory(key, parent), vtbl(vtbl) {}
...
VirtualQSharedMemory* QSharedMemory_new_QNativeIpcKey(const QSharedMemory_VTable* vtbl, size_t vdata, QNativeIpcKey* key) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSharedMemory>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSharedMemory(vtbl, *key) : nullptr;
}

VirtualQSharedMemory* QSharedMemory_new_QString(const QSharedMemory_VTable* vtbl, size_t vdata, struct seaqt_string key) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSharedMemory>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSharedMemory(vtbl, key_QString) : nullptr;
}

VirtualQSharedMemory* QSharedMemory_new_QObject(const QSharedMemory_VTable* vtbl, size_t vdata, QObject* parent) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSharedMemory>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSharedMemory(vtbl, parent) : nullptr;
}

VirtualQSharedMemory* QSharedMemory_new_QNativeIpcKey_QObject(const QSharedMemory_VTable* vtbl, size_t vdata, QNativeIpcKey* key, QObject* parent) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSharedMemory>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQSharedMemory(vtbl, *key, parent) : nullptr;
}

VirtualQSharedMemory* QSharedMemory_new_QString_QObject(const QSharedMemory_VTable* vtbl, size_t vdata, struct seaqt_string key, QObject* parent) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQSharedMemory>() + vdata, std::nothrow);
...
void QSharedMemory_setNativeKey_QNativeIpcKey(QSharedMemory* self, QNativeIpcKey* key) {
	self->setNativeKey(*key);
}

void QSharedMemory_setNativeKey_QString(QSharedMemory* self, struct seaqt_string key) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
...
QNativeIpcKey* QSharedMemory_platformSafeKey_key(struct seaqt_string key) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
	return new QNativeIpcKey(QSharedMemory::platformSafeKey(key_QString));
}

QNativeIpcKey* QSharedMemory_legacyNativeKey_key(struct seaqt_string key) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
...
void QSharedMemory_setNativeKey_QString_QNativeIpcKey_Type(QSharedMemory* self, struct seaqt_string key, uint16_t type) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
...
QNativeIpcKey* QSharedMemory_platformSafeKey_key_type(struct seaqt_string key, uint16_t type) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
	return new QNativeIpcKey(QSharedMemory::platformSafeKey(key_QString, static_cast<QNativeIpcKey::Type>(type)));
}

QNativeIpcKey* QSharedMemory_legacyNativeKey_key_type(struct seaqt_string key, uint16_t type) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
