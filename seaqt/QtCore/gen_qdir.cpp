...
void QDir_operatorAssign_from(QDir* self, QDir* from) {
	self->operator=(*from);
}

void QDir_operatorAssign_path(QDir* self, struct seaqt_string path) {
	QString path_QString = QString::fromUtf8(path.data, path.len);
...
void QDir_setSearchPaths(struct seaqt_string prefix, struct seaqt_array /* of struct seaqt_string */  searchPaths) {
	...
	struct seaqt_string* searchPaths_arr = static_cast<struct seaqt_string*>(searchPaths.data);
