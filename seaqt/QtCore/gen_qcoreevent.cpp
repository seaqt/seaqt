...
public:
	...
	VirtualQTimerEvent(const QTimerEvent_VTable* vtbl, Qt::TimerId timerId): QTimerEvent(timerId), vtbl(vtbl) {}
...
VirtualQTimerEvent* QTimerEvent_new_Qt_TimerId(const QTimerEvent_VTable* vtbl, size_t vdata, int timerId) {
	void* _mem_ = ::operator new(seaqt_aligned_sizeof<VirtualQTimerEvent>() + vdata, std::nothrow);
	return _mem_ ? new (_mem_)VirtualQTimerEvent(vtbl, static_cast<Qt::TimerId>(timerId)) : nullptr;
}
