...
QTextStream* QTextStream_new_cQByteArray_QIODevice_OpenMode(struct seaqt_string array, int openMode) {
	QByteArray array_QByteArray(array.data, array.len);
	return new (std::nothrow) QTextStream(array_QByteArray, static_cast<QIODevice::OpenMode>(openMode));
}

void QTextStream_setCodec_codec(QTextStream* self, QTextCodec* codec) {
	self->setCodec(codec);
}

void QTextStream_setCodec_codecName(QTextStream* self, const char* codecName) {
