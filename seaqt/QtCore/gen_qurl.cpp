...
QUrl* QUrl_fromEncoded_input(struct seaqt_string input) {
	QByteArrayView input_QByteArray(input.data, input.len);
	return new QUrl(QUrl::fromEncoded(input_QByteArray));
}
...
QUrl* QUrl_fromEncoded_input_mode(struct seaqt_string input, int mode) {
	QByteArrayView input_QByteArray(input.data, input.len);
	return new QUrl(QUrl::fromEncoded(input_QByteArray, static_cast<QUrl::ParsingMode>(mode)));
}
